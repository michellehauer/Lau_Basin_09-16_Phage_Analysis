...
#How far out the lables are
    ...
    if colorlabels != None:
        ...
        j = 0
        outerrad = R * 1.05 + width * len(colorlabels) + space * (len(colorlabels) - 1) + 0.15
        ...
        for labelname, colorlist in colorlabels.items():
            ...
            patches, texts = ax.pie(
                ...
                wedgeprops=dict(
                    width=width,
                    linewidth = 0,
                    #edgecolor='g', #if this is active the wedges will be more clearly separated
                ),
            )
...
def radialTreee3(
    ...
    if addlabels:
        ...
        for (_x, _y, _rot), label in zip(label_coords, Z2["ivl"]):
            ax.text(
                _x, _y, label, {"va": "center"},
                rotation_mode="anchor", rotation=_rot, fontsize=fontsize,
            )

    def calculate_intervals(coords):
        intervals = []
        for i in range(len(coords)):
            _xl, _yl, _ = coords[i - 1]
            _x, _y, _ = coords[i]
            if i == len(coords) - 1:
                _xr, _yr, _ = coords[0]
            else:
                _xr, _yr, _ = coords[i + 1]
            d = ((_xr - _xl) ** 2 + (_yr - _yl) ** 2) ** 0.5
            intervals.append(d)
        return intervals

    intervals = calculate_intervals(label_coords)
    colorpos = intervals
    ...
    if colorlabels is not None:
        ...
        outerrad = R * 1.05 + (width + space) * (len(colorlabels)) - space + 0.15

        labelnames = []
        for j, (labelname, colorlist) in enumerate(colorlabels.items()):
            colorlist = np.array(colorlist)[Z2["leaves"]]
            outerrad -= width + space
            patches, texts = ax.pie(
                colorpos,
                colors=colorlist,
                radius=outerrad,
        ...
        if colorlabels_legend is not None:
            for i, labelname in enumerate(labelnames):
                colorlines = [Line2D([0], [0], color=c, lw=4) for c in colorlabels_legend[labelname]["colors"]]
                leg = ax.legend(
                    colorlines,
                    colorlabels_legend[labelname]["labels"],
                    bbox_to_anchor=(1.5 + 0.3 * i, 1.0),
        ...
        outerrad = R * 1.05 + (width + space) * (len(sample_classes)) - space

        labelnames = []
        colorlabels_legend = {}
        colormap_list = ["gist_rainbow", "viridis", "plasma", "inferno", "magma"]
        for j, (labelname, colorlist) in enumerate(sample_classes.items()):
            ucolors = sorted(list(np.unique(colorlist)))
            type_num = len(ucolors)
            _cmp = cm.get_cmap(colormap_list[j % len(colormap_list)], type_num)
            _colorlist = [_cmp(ucolors.index(c)) for c in colorlist]
            _colorlist = np.array(_colorlist)[Z2["leaves"]]
            outerrad -= width + space
            patches, texts = ax.pie(
    ...
    if colorlabels is not None:
        maxr = R * 1.05 + (width + space) * (len(colorlabels)) - space
    elif sample_classes is not None:
        maxr = R * 1.05 + (width + space) * (len(sample_classes)) - space
    else:
        maxr = R * 1.05

## Original code from dRep: https://github.com/MrOlm/drep/blob/master/drep/d_analyze.py
## Using a forked radialtree (see radialtree_fork.py) to circularize the final dendrogram
## Original Author: MrOlm, Matt Olm
...
def plot_secondary_dendrograms_from_wd(wd, plot_dir, **kwargs):
    ...
    for cluster in sorted(Cdb['primary_cluster'].unique()):
        ...
        # Make the dendrogram
        g, Z2 =_make_special_dendrogram(linkage, names, **kwargs)
...
def fancy_dendrogram(linkage,names,name2color=False,threshold=False,self_thresh=False):
    ...
    if threshold == False:
       ...
       Z2 = scipy.cluster.hierarchy.dendrogram(linkage,labels=names, color_threshold=threshold,\
                                            orientation='right')
...
def _make_special_dendrogram(linkage, names, **kwargs):
    ...
    # Make the dendrogram
    sns.set_style('whitegrid')
    g, Z2 = fancy_dendrogram(linkage,names,name2color,threshold=threshold,self_thresh =\
                        self_thresh)
...
# Create the circular dendrogram function
def circular_dendrogram(linkage_matrix, plot_dir, **kwargs):
 # Initialize a .pdf
    if plot_dir != False:
        pp = PdfPages(plot_dir + 'dendy.pdf')
        save = True
    else:
        save = False
        
    # Compute the dendrogram and don't plot it
    R = dendrogram(linkage_matrix, no_plot=True)

    # Create the figure
    fig, ax = plt.subplots(figsize=(8, 8), subplot_kw=dict(polar=True))

    # Compute the angles for each leaf
    icoord = np.array(R['icoord'])
    dcoord = np.array(R['dcoord'])
    leaves = np.array(R['leaves'])
    n = len(leaves)
    angle = np.pi * (1 - 2*leaves / n)
    radius = dcoord.max() + dcoord[0][1]

    # Plot each line segment
    for i, (xs, ys) in enumerate(zip(icoord, dcoord)):
        xs = np.pi * (1 - 2*xs / n)
        ys = radius - ys
        lines = LineCollection([list(zip(xs, ys))], color='k')
